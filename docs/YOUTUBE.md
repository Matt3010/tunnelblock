...
The scanner therefore ranks the nearest verified length-delimited field enclosing each marker and records distance statistics. It also keeps the marker identity separate and derives a bounded nearest-to-outer ancestor chain for each marker, so a capture can distinguish patterns such as `pagead -> field 14` from `googleadservices -> field 7` and identify any parent fields shared by both branches. Broader candidate lists remain diagnostic because nested protobuf messages and random schema-free byte patterns can produce multiple plausible ancestors.
