...
A field-denaturing implementation is available for the later blocking experiment, but it now uses structural matching: a configured field number is eligible only when the same physical protobuf node contains every configured ad marker type. This prevents a same-number leaf containing only `/pagead/` from being mutated accidentally. The checked-in defaults remain observation-only, while `scripts/protobuf-mutation.sh enable <field>` can create an explicit runtime-only test session; `disable` restores HTTPS interception off and QUIC allowed. Historical field numbers are not trusted automatically; current iOS observations must validate them first.
