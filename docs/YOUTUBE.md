...
A field-denaturing implementation is available for the later blocking experiment, but it now uses structural matching: a configured field number is eligible only when the same physical protobuf node contains every configured ad marker type. Diagnostic reporting and mutation now consume the same selected node plan from the same scanner instance. The runtime records both planned and mutated field counts and rejects the mutation if they differ, leaving the original response untouched. This prevents a broader schema-free rescan from mutating nodes that were not present in the dry-run report. The checked-in defaults remain observation-only, while `scripts/protobuf-mutation.sh enable <field>` can create an explicit runtime-only test session; `disable` restores HTTPS interception off and QUIC allowed.
