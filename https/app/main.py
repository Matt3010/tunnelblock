...
if MODE not in {"disabled", "observe", "filter"}:
    raise ValueError("HTTPS_MODE must be disabled, observe or filter")
if MODE == "filter" and (_strategy is None or "filter" not in _strategy.config.actions):
    raise ValueError("active strategy does not support filter mode")
...
def requestheaders(flow: http.HTTPFlow) -> None:
    ...
    extra = _strategy.on_request(context) if _strategy else {}
    _emit(
        "http_request",
        host=context.host,
...
def responseheaders(flow: http.HTTPFlow) -> None:
    ...
    extra = _strategy.on_response(context) if _strategy else {}
    _emit(
        "http_response",
        host=context.host,
        method=context.method,
